...
function LeaveMod({ user, users, leaves, setLeaves, toast, openRecordId, onRecordOpened }) {
  ...
  const addLeave=async (f)=>{
    ...
    // Notify first approver (dept head / LO) that a leave request is waiting
    const firstApprovers = users.filter(u => {
      if(user.category==="pastor") return u.role==="lo" && u.lcc_overseen===user.lcc && u.approved;
      return deptHeadRole(deptForUser) && u.role===deptHeadRole(deptForUser) && u.approved;
    });
    firstApprovers.forEach(a => sendGenericEmail({
      to_email: a.email,
      to_name: a.name,
...
function StaffProf({ staff, user, users, canEdit, canEditDetails, lccs, onClose, onUpdate, onTransfer, onAddUser, onUpdateUser, onGetUsers }) {
  ...
  const isAlreadyLO = (staff.appointment?.role==="lo" && staff.appointment?.active) || !!staff.loAppointment?.active;
    ...
    <div className="overlay">
      <div className="modal" style={{maxWidth:720}}>
        ...
        <div style={{padding:24}}>
          ...
          {/* LO credentials shown on pastor's own profile — same as other appointments, handled by _apptTempPw block above */}
              ...
              <div style={{background:"#fff",borderRadius:8,padding:"12px 14px",display:"grid",gap:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div>
                    ...
                    <div style={{fontSize:14,fontWeight:700,color:"#0b1f3a"}}>{staff._apptTempEmail}</div>
                  </div>
                  <button onClick={()=>navigator.clipboard?.writeText(staff._apptTempEmail||"").then(()=>alert("Email copied!"))}
                ...
                <div style={{borderTop:"1px solid #f5f5f5",paddingTop:10,display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div>
                    ...
                    <div style={{fontSize:16,fontWeight:700,color:"#0b1f3a",fontFamily:"monospace",letterSpacing:2}}>{staff._apptTempPw}</div>
                  </div>
                  <button onClick={()=>navigator.clipboard?.writeText(staff._apptTempPw||"").then(()=>alert("Password copied!"))}
                    style={{background:"#fef9ee",border:"1px solid #f0d060",color:"#7d6008",borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:600}}>📋 Copy</button>
                </div>
              </div>
              <div style={{fontSize:11,color:"#c9a84c",marginTop:10,fontStyle:"italic"}}>⚠️ You will be asked to change this password on first login as {staff._apptTempRole}.</div>
            </div>
          )}
          ...
          {/* Appointment badge visible to others */}
          {staff.appointment?.active && staff.appointment?.role!=="lo" && staff.id!==user.id&&(
            <div style={{background:"#fef9ee",border:"1px solid #f0d060",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#7d6008"}}>
              👑 <strong>{staff.name}</strong> is currently serving as <strong>{staff.appointment.label}</strong> since {fdate(staff.appointment.appointedOn)}
            </div>
          )}
...
function SignUp({ users, lccs, setLccs, onSignUp, onGo }) {
  ...
  const go=async ()=>{
    ...
    try {
      ...
      // Notify the new user their request is received
      sendGenericEmail({
        to_email: f.email,
        to_name: f.name,
        email_subject: "Account Request Received — ECWA Lafia DCC Portal",
        email_body: `Dear ${f.name},\n\nThank you for registering on the ECWA Lafia DCC Staff Management Portal.\n\nYour account request has been received and is currently pending approval by Admin & Personnel. You will receive another email as soon as your account is approved.\n\nPortal: https://ecwa-portal.onrender.com\n\nGod bless you.`,
      });
      // Notify Admin & Personnel that a new account is waiting for approval
      const admins = users.filter(u=>["personnel","admin","conf_secretary"].includes(u.role) && u.approved);
      admins.forEach(a => sendGenericEmail({
        to_email: a.email,
        to_name: a.name,
        email_subject: "New Account Pending Approval — ECWA Lafia DCC Portal",
        email_body: `Dear ${a.name},\n\nA new account registration is awaiting your approval on the ECWA Lafia DCC Staff Management Portal.\n\nName: ${f.name}\nEmail: ${f.email}\nCategory: ${cat==="pastor"?"Pastor":"Office Staff"}\n\nPlease log in to review and approve or reject this request.\n\nPortal: https://ecwa-portal.onrender.com`,
      }));
      setOk(true);
    } catch(e) {
...
function MasterAppointments({ users, setUsers, toast, addLog }) {
  ...
  const genPw = () => {
    ...
    let p="ECWA@"; for(let i=0;i<6;i++) p+=c[Math.floor(Math.random()*c.length)]; return p;
  };

  const appoint = async (pastor, position) => {
    const current   = currentHolders[position.role];
    const apptEmail = APPT_EMAIL[position.role];
    const tempPw    = genPw();
    const hashed    = await hashPassword(tempPw);
    const apptOn    = today();

    setUsers(us => {
      // Find existing office account for this role (may exist from a previous appointment)
      const existingAcct = us.find(u => u.role===position.role && u.category==="office" && u._apptAccount);
      ...
      return us.map(u => {
        ...
        // 2. Update or suspend existing office account
        if(existingAcct && u.id===existingAcct.id) {
          // Reactivate with new appointee's biodata
          return { ...u,
        ...
        // 4. If no existing office account, create one fresh
        !existingAcct ? [{
          id:"APPT_"+position.role,
                    ...
                    try {
                      ...
                      if(error) toast("⚠️ Approved locally but DB save failed: "+error.message,"danger");
                        ...
                        sendGenericEmail({
                          to_email: u.email,
                          to_name: u.name,
                          email_subject: "Your Account Has Been Approved — ECWA Lafia DCC Portal",
                          email_body: `Dear ${u.name},\n\nWe are pleased to inform you that your account on the ECWA Lafia DCC Staff Management Portal has been approved. You can now sign in using your registered email and password.\n\nPortal: https://ecwa-portal.onrender.com\n\nIf you have any issues signing in, please contact Admin & Personnel.\n\nGod bless you.`,
                        });
                      }
                  ...
                  <button onClick={async ()=>{
                    ...
                    try {
                      ...
                      sendGenericEmail({
                        to_email: u.email,
                        to_name: u.name,
                        email_subject: "Your Account Request — ECWA Lafia DCC Portal",
                        email_body: `Dear ${u.name},\n\nThank you for your interest in the ECWA Lafia DCC Staff Management Portal.\n\nUnfortunately, your account request could not be approved at this time. If you believe this is an error or would like further clarification, please contact Admin & Personnel directly.\n\nGod bless you.`,
                      });
